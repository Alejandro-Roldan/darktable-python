...
def get_xmp_color_labels(file) -> set[darktable.ColorLabel]:
    namespaces = dict(
        [node for _, node in ElementTree.iterparse(file, events=["start-ns"])]
    )
...
def format_color_labels(color_labels: set[darktable.ColorLabel]):
    items = list(
        map(lambda c: c.name.lower(), sorted(list(color_labels), key=lambda c: c.value))
    )
...
def scan(photos, percent_step):
    ...
    print(
        "scanning database and xmp files. this could take a while",
        end="",
        file=sys.stderr,
    )
    ...
    for i, photo in enumerate(photos):
        ...
        if xmp_rating is None:
            ...
            result_no_xmp_rating.append(
                f"{photo_filename}: no rating in xmp file. {format_info(copy)}"
            )
        ...
        if xmp_rating is not None and xmp_rating != database_rating:
            ...
            result_inconsistent_xmp_rating.append(
                f"{photo_filename}: inconsistent xmp rating. {format_info(copy)}"
            )
        ...
        if database_color_labels != xmp_color_labels:
            ...
            result_inconsistent_xmp_labels.append(
                f"{photo_filename}: inconsistent xmp color labels. {format_info(copy)}"
            )
...
def main():
    if len(sys.argv) != 2:
        print(
            "first argument must be the path to your Darktable config directory that "
            "contains library.db and data.db",
            file=sys.stderr,
        )
        return -1
    ...
    n += print_result(
        "xmp rating inconsistent from database rating", result_inconsistent_xmp_rating
    )
    n += print_result(
        "xmp color labels inconsistent from database color labels",
        result_inconsistent_xmp_labels,
    )

    print(f"{len(photos)} total photos in library")
    print(
        f"{count_checked} photos checked that either have a rating above "
        f"{MIN_RATING_EXCLUDED} or color labels in either the database or the xmp file"
    )
    if len(result_no_xmp) > 0:
        print(f"WARN {len(result_no_xmp)} photos have no xmp file")
    if len(result_no_xmp_rating) > 0:
        print(
            f"WARN {len(result_no_xmp_rating)} photos have no rating in their xmp file"
        )
    if len(result_inconsistent_xmp_rating) > 0:
        print(
            f"WARN {len(result_inconsistent_xmp_rating)} photos have an xmp rating that"
            " is different from their database rating"
        )
    if len(result_inconsistent_xmp_labels) > 0:
        print(
            f"WARN {len(result_inconsistent_xmp_labels)} photos have color labels that "
            "are inconsistent with their database color labels"
        )
    if n == 0:
        print("GOOD your database and xmp files look consistent!")
    else:
